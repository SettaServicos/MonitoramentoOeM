...
class PVOperation:
    ...
    def buscar_potencia_instantanea(self, usina_id: str, full_day_check=False) -> tuple[list, list]:
        ...
        Parâmetros:
            full_day_check: Se True, verifica desde 6h30 da manhã até agora
        """
        print(f"⚡ Buscando potência para usina {usina_id} ({'desde 6h30' if full_day_check else 'últimos 15 min'})")
        
        # 1. Define o período de análise
        agora = datetime.now()
        if full_day_check:
            # Começa às 6h30 do dia atual (ou do dia anterior se for antes das 6h30)
            inicio_periodo = agora.replace(hour=6, minute=30, second=0, microsecond=0)
            if agora.hour < 6.5:
